...
from bdp.common.tensorboard_utils import TensorboardWriter, get_writer
from bdp.rl.ddppo.ddp_utils import SAVE_STATE, is_slurm_batch_job
from bdp.utils.common import get_checkpoint_id, poll_checkpoint_folder
...
class BaseTrainer:
    ...
    def eval(self) -> None:
        ...
        with get_writer(self.config, flush_secs=self.flush_secs) as writer:
            if self.config.EVAL_CKPT_PATH_DIR is None:
...
class BaseRLTrainer(BaseTrainer):
    ...
    def _pause_envs(
        ...
        if len(envs_to_pause) > 0:
            ...
            all_prev_actions = [
                prev_action[state_index] for prev_action in all_prev_actions
            ]
