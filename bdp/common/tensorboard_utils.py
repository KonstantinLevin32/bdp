...
class TensorboardWriter:
    def __init__(self, log_dir: str, *args: Any, resume_run_id=None, **kwargs: Any):
...
class WeightsAndBiasesWriter:
    def __init__(self, config, *args: Any, resume_run_id=None, **kwargs: Any):
        r"""
        ...
        def get_git_diff(rel_dir, save_name):
            wd = os.getcwd()
            os.chdir(rel_dir)
            proc = subprocess.Popen(["git diff"], stdout=subprocess.PIPE, shell=True)
            (gitdiff, err) = proc.communicate()
            fname = os.path.join(wandb.run.dir, save_name + ".txt")
            proc = subprocess.Popen(
                ["git rev-parse HEAD"],
                stdout=subprocess.PIPE,
                shell=True,
            )
            (commit_hash, err) = proc.communicate()
            with open(fname, "w") as f:
                f.write(gitdiff.decode("utf-8"))
                f.write("\n")
        ...
        hablab_diff_f = get_git_diff("../habitat-lab", "hablab_diff")
        cur_diff_f = get_git_diff(".", "cur_diff")
        wandb.save(hablab_diff_f)
        wandb.save(cur_diff_f)
