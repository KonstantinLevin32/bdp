            ...
            for k in self.keys():
                ...
                if isinstance(v, (TensorDict, dict)):
                    ...
                    tgt.set(
                        ...
                        v,
                        strict=strict,
                        alternate_assign=alternate_assign,
                    )
                else:
                    assert isinstance(tgt, torch.Tensor)
                    if alternate_assign:
                        tgt[index] = tgt[index].copy_(torch.as_tensor(v))
                    else:
    ...
    def map_func(
        ...
        for k, v in src.items():
            if isinstance(v, torch.Tensor):
                ...
                assert sub_dst is None or isinstance(
                    sub_dst, TensorDict
                ), f"Got type {type(sub_dst)}"
