...
class ResetArmSkill(SkillPolicy):
    ...
    def _is_skill_done(self, observations, rnn_hidden_states, prev_actions, masks):
    ...
    def _internal_act(
        ...
        full_action[..., self._ac_start : self._ac_start + 7] = torch.from_numpy(
            delta
        ).to(device=full_action.device, dtype=full_action.dtype)
