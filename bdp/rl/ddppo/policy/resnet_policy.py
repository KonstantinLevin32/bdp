...
from bdp.rl.ddppo.policy.running_mean_and_var import RunningMeanAndVar
from bdp.rl.models.rnn_state_encoder import build_rnn_state_encoder
from bdp.rl.ppo import Net, NetPolicy
...
class PointNavResNetNet(Net):
    ...
    def forward(
        ...
        if len(self._fuse_keys) != 0:
            fuse_states = torch.cat([observations[k] for k in self._fuse_keys], dim=-1)
